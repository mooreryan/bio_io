...
let total_length () =
  let total_length =
    Fasta.In_channel.with_file_fold_records_exn infile ~init:0
      ~f:(fun length record ->
...
let print_records () =
  Fasta.In_channel.with_file_iter_records_exn infile ~f:(fun record ->
      let open Fasta.Record in
      eprintf "%s => %d\n" (id record) (String.length (seq record)))

let print_recordsi () =
  Fasta.In_channel.with_file_iteri_records_exn infile ~f:(fun index record ->
      let open Fasta.Record in
      eprintf "%d: %s => %d\n" (index + 1) (id record)
